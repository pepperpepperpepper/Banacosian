- Buttons & selects share near-identical “pill” styling in two places: `css/components/controls.css:1-73` (`.ui-pill`, `.select-pill`) and `www/staff/styles.css:123-154` (`.font-select, button`). You can drop the latter in favour of the shared `.ui-pill` class (e.g., add `class="ui-pill"` in the font demo) and expose any font-demo-specific tweaks through additional modifiers instead of duplicating the base styles.
...
- Responsive rules for the piano keys show up in both `css/responsive.css:1-14` (main app) and `www/staff/styles.css:206-228` (font demo) with only minor differences. Extract a single breakpoint block (perhaps in a shared `css/responsive/piano.css`) so the keyboard height math is maintained in one place.
