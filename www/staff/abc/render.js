import { readTokens } from '../shared/theme.js';
import { getKeySignatureAlteration } from '/js/modules/KeySignatures.js';
import { waitForABCJS } from './loader.js';
import {
  ABC_SNIPPETS,
  getSnippet,
  getStoredSnippet,
...
export function configureRenderContext({ staffContainer, keySelect, statusEl, smuflRenderer }) {
  renderContext.staffContainer = staffContainer || null;
  renderContext.keySelect = keySelect || null;
  renderContext.statusEl = statusEl || null;
  renderContext.smuflRenderer = smuflRenderer || null;
}
...
function commitWheelSemitoneDelta(delta) {
  const selection = getLastSelected();
  const key = getActiveKey();
  if (!selection || !key) return;
  const abc = getSnippet(key);
  if (!abc) return;
  const tok = findTokenByCharRange(abc, selection.start, selection.end);
  if (!tok) return;
  ...
  setSnippet(key, updated);
  setLastSelected({ key, start: before.length, end: before.length + replacement.length });
...
export async function renderSelection(key, opts = {}) {
  setActiveKey(key);
  if (getIsLoading()) return;
  setIsLoading(true);
  if (renderContext.statusEl) {
    renderContext.statusEl.textContent = 'Rendering…';
  }

  try {
    const ABCJS = await waitForABCJS();
    const stored = getStoredSnippet(key);
    const defaultSnippet = getDefaultSnippet(key) ?? stored ?? null;
    const snippet = (opts.useExisting !== false)
      ? (stored ?? defaultSnippet)
      : (defaultSnippet ?? stored);
    if (!snippet) {
      throw new Error(`No ABC snippet defined for key "${key}"`);
    }

    const staffContainer = renderContext.staffContainer;
    if (!staffContainer) {
      throw new Error('No staff container configured for renderSelection.');
    }
    ...
    try {
      ...
      const bindPromises = [];
      const { smuflRenderer } = renderContext;
      svgs.forEach((s) => {
        if (smuflRenderer?.bindSvg) {
        ...
        const startDragFromEvent = (ev, groupOverride = null) => {
          ...
          const basePitchText = tok.text;
          const baseDuration = tok.dur || '';
          const baseChrom = abcTokenToChromaticIndex(tok.text, key);
          const pointerId = ev.pointerId != null ? ev.pointerId : null;
          const overlay = ensureNoteOverlay(s);
          const smuflNote = smuflRenderer?.getNoteForGroup ? smuflRenderer.getNoteForGroup(group) : null;
          if (smuflNote?.beginDrag) {
            try { smuflNote.beginDrag(); } catch (_err) { /* ignore */ }
          }
          const nextState = {
            key,
            startChar: tok.start,
            endChar: tok.end,
            startChrom: baseChrom,
            accum: 0,
            lastY: pointerY,
          ...
          if (pointerId != null && typeof group.setPointerCapture === 'function') {
            try { group.setPointerCapture(pointerId); } catch (_err) { /* ignore */ }
          }
        ...
        const notes = s.querySelectorAll('g.abcjs-note');
        notes.forEach((g) => {
          ...
          g.addEventListener('pointerdown', pointerHandler);
          g.addEventListener('mousedown', pointerHandler);
        ...
        const containerHandler = (ev) => {
          startDragFromEvent(ev);
        };
        s.addEventListener('pointerdown', containerHandler);
        s.addEventListener('mousedown', containerHandler);
    ...
    if (renderContext.statusEl) {
      renderContext.statusEl.textContent = '';
    }
  } catch (error) {
    console.error(error);
    if (renderContext.statusEl) {
      renderContext.statusEl.textContent = 'Unable to render staff demo. Check console for details.';
    }
  } finally {
    setIsLoading(false);
  }
